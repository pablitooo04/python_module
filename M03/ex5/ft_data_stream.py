...
if __name__ == "__main__":
    ...
    for i in range(len(events)):
        ...
        if event["event_type"] == "kill":
            event_message = "killed monster"
        elif event["event_type"] == "item_found":
            event_message = "found treasure"
        elif event["event_type"] == "level_up":
            event_message = "leveled up"
        else:
            event_message = event["event_type"]
        print(f"Event {i}: Player {event['player']}", end="")
        print(f" (level {event['data']['level']}) {event_message}")

    treasure_number = len([data for data in
                           events if data["event_type"] == "item_found"])
    level_up_number = len([data for data in
                           events if data["event_type"] == "level_up"])
    high_level = len([data["data"]["level"]for data in
                      events if data["data"]["level"] > 10])

    print("\n=== Stream Analytics ===")
    print(f"Total events processed: {len(events)}")
    print(f'High-level players (10+): {high_level}')
    print(f'Treasure events: {treasure_number}')
    print(f'Level-up events: {level_up_number}')
    print("Memory usage: Constant (streaming)")
    print(f"Processing time: {round(time() - time_a, 5)} seconds\n")
    print("=== Generator Demonstration ===\n")

    sequence = fibonacci()
    next_sequence = [next(sequence) for i in range(10)]
    print(f"Fibonacci sequence : (first 10): {next_sequence}")

    prime = prime_numbers()
    print(f"Prime numbers : (first 5): {[next(prime) for i in range(5)]}")
